...
public class ArmaController : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
{
    public Arma item;
    ...
    private void Start()
    {
        ...
        skin = transform.GetChild(0).GetChild(0).gameObject.GetComponent<actualizarArma>();
    ...
    {
        item = new Arma(nombreEquipamiento, equipo, tipoCalidad);
    }

    //ITEM INVENTARIO
    public void inicializa(Arma item, bool nuevo)
    {
        this.item = item;
    ...
    //ITEM INVENTARIO
    public void inicializa(Arma item)
    {
        this.item = item;
        Debug.Log(item);
        Debug.Log(this.item.Danno);
    }

    //ITEM INVENTARIO DESDE JSON
    public void loadJson(Arma item)
    {
        this.item = item;
        transform.GetChild(0).gameObject.SetActive(true);
        transform.GetChild(0).GetChild(0).gameObject.SetActive(true);
    ...
    {
        item = new Arma(nombre, equipo, "Comun (Instance)");
