...
public class ItemController : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler
{
    ...
    private void Start()
    {
        ...
        skin = transform.GetChild(0).GetChild(0).gameObject.GetComponent<actualizarItem>();
    ...
    //ITEM INVENTARIO
    public void inicializa(Item item, bool nuevo)
    {
        inicializa(item);
    ...
    //ITEM INVENTARIO
    public void inicializa(Item item)
    {
        if (item is Armadura) {
            this.item = new Armadura(item);
        } else if (item is Arma) {
            this.item = new Arma(item);
        } else {
    ...
    public void loadJson(Item item)
    {
        this.item = new Armadura(item);
        transform.GetChild(0).gameObject.SetActive(true);
        transform.GetChild(0).GetChild(0).gameObject.SetActive(true);
